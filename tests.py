...
from django_elasticsearch.models import EsIndexable, es
...
class EsIndexableTestCase(TestCase):
    def setUp(self):
        ...
        TestModel.es_create_index()
        self.instance = TestModel.objects.create(username=u"1", first_name=u"woot", last_name=u"foo")
        self.instance.es_do_index()
        self.instance.es_do_update()

    def tearDown(self):
        self.instance.delete()
        self.instance.es_delete()
        es.indices.delete(index='django-test')

    def test_serialize(self):
        json = self.instance.es_serialize()
    ...
    def test_do_index(self):
        self.instance.es_do_index()
        r = self.instance.es_get()
        self.assertTrue(r['found'])

    def test_delete(self):
        self.instance.es_delete()
        r = self.instance.es_get(ignore=404)
        self.assertFalse(r['found'])

    # TODO: this test fails, i don't know why
    @SkipTest
    def test_mlt(self):
        results = self.instance.es_mlt(fields=['first_name',])
    ...
    def test_search(self):
        hits = TestModel.es_search('wee')
        self.assertEqual(len(hits), 0)

        hits = TestModel.es_search('woot')
        self.assertEqual(len(hits), 1)

    def test_search_with_facets(self):
        s = TestModel.es_search('whatever', facets=['first_name',])
    ...
    def test_custom_mapping(self):
        ...
        self.assertEqual(expected, TestModel.es_make_mapping())
...
try:
    ...
    class EsTaggitTestCase(TestCase):
        def test_serializer(self):
            TestModel.es_serializer_class = TaggitSerializer
